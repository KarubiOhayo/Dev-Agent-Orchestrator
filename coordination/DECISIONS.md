...
## D-050 fallback-warning H-037 승인 및 H-038 seeding 실패 패턴 추적/완화 가이드 정합화 정책
- Date: 2026-02-20
- Status: Approved (H-037 Close-out / H-038 Scope)
- Decision:
  - H-037 결과를 기준으로 Main 최종 판단을 `Go`로 확정한다.
  - H-037 재집계 결과에서도 `INSUFFICIENT_SAMPLE_RATIO=0.9286`, `SUFFICIENT_DAYS=1`로 재개 게이트 2종이 미충족이므로 `resumeDecision=KEEP_FROZEN`을 유지한다.
  - 다음 라운드(H-038)는 fail-fast 유지 반복 시딩으로 `parseEligibleRunCount` 누적/게이트 재집계를 이어가고, 체인 실패 케이스의 모델 후보 실패 원인(`temperature` 파라미터 비호환, 모델 미존재, all candidates failed) 재발 빈도와 완화 가이드 반영 여부를 운영 문서에서 정합화한다.
  - H-038에서도 fallback-warning 운영 계약 필드(`signalRecoveryEvidenceLedger[]`, `evidenceAccumulationSummary[]`, `evidenceFreshnessSummary[]`), 단일 판정(`RESUME_H024|KEEP_FROZEN`), 임계치/알림 룰 수치(`0.05`, `0.15`, `+0.10p`, `0.10`) 및 `INSUFFICIENT_SAMPLE` 제외 규칙은 변경하지 않는다.
- Rationale: H-037에서 워크트리 위생 이슈는 해소되고 실행량/체인 증거가 추가 누적됐지만 재개 게이트 미충족이 지속되어 추적 라운드를 연속해야 하며, fail-fast 실패 원인 분류/완화 기준을 문서로 고정해야 라운드 간 재현 가능한 운영 판단이 가능하기 때문이다.
- Consequence:
...
## D-056 H-039 승인(Go) 확정 및 H-042 후속 점검 정책
- Date: 2026-02-23
- Status: Approved (H-039 Close-out / H-042 Scope)
- Decision:
  - H-039 결과를 Main 최종 판단 `Go`로 승인한다.
  - H-039 재집계 결과(`INSUFFICIENT_SAMPLE_RATIO=0.8571`, `SUFFICIENT_DAYS=2`, `executionGapDelta=-135`, `chainShareGapDelta=-43.57%p`)를 기준으로 `resumeDecision=KEEP_FROZEN`을 유지한다.
  - 다음 실행 라운드는 H-042로 고정하고, 최신 시딩 누적/게이트 재집계 + H-036~H-039/H-042 readiness 추세 비교로 `RESUME_H024|KEEP_FROZEN` 단일 판정을 재검증한다.
  - fallback-warning 운영 계약 필드(`signalRecoveryEvidenceLedger[]`, `evidenceAccumulationSummary[]`, `evidenceFreshnessSummary[]`), 임계치/알림 룰 수치(`0.05`, `0.15`, `+0.10p`, `0.10`), `INSUFFICIENT_SAMPLE` 제외 규칙은 변경하지 않는다.
- Rationale: H-039에서 일부 개선 신호(`INSUFFICIENT_SAMPLE_RATIO` 하락, `SUFFICIENT_DAYS` 증가, `executionGapDelta` 개선)는 확인됐지만 재개 게이트 4종 중 2종이 미충족이며 최근 3일 평균 전체 모수(`9.0000`)가 기준(`>=32`)에 미달해 H-024 재개 근거가 아직 부족하기 때문이다.
- Consequence:
...
## D-058 H-044 승인(Go) 확정 및 H-045 후속 점검 정책
- Date: 2026-02-26
- Status: Approved (H-044 Close-out / H-045 Scope)
- Decision:
  - H-044 결과를 Main 최종 판단 `Go`로 승인한다.
  - H-044 재집계 결과(`INSUFFICIENT_SAMPLE_RATIO=0.7857`, `SUFFICIENT_DAYS=3`, `executionGapDelta=-208`, `chainShareGapDelta=-46.41%p`, 최근 3일 평균 전체 `parseEligibleRunCount=7.6667`)를 기준으로 `resumeDecision=KEEP_FROZEN`을 유지한다.
  - 다음 실행 라운드는 H-045로 고정하고, 최신 시딩 누적/게이트 재집계 + H-036~H-039/H-042/H-043/H-044/H-045 readiness 추세 비교로 `RESUME_H024|KEEP_FROZEN` 단일 판정을 재검증한다.
  - fallback-warning 운영 계약 필드(`signalRecoveryEvidenceLedger[]`, `evidenceAccumulationSummary[]`, `evidenceFreshnessSummary[]`), 임계치/알림 룰 수치(`0.05`, `0.15`, `+0.10p`, `0.10`), `INSUFFICIENT_SAMPLE` 제외 규칙은 변경하지 않는다.
- Rationale: H-044에서 일부 개선 신호(`INSUFFICIENT_SAMPLE_RATIO` 하락, `SUFFICIENT_DAYS` 증가)는 이어졌지만, 재개 게이트 4종 중 2종(`INSUFFICIENT_SAMPLE_RATIO`, `SUFFICIENT_DAYS`)이 여전히 미충족이고 최근 3일 평균 전체 모수(`7.6667`)가 기준(`>=32`)을 크게 하회해 H-024 재개 근거가 아직 부족하기 때문이다.
- Consequence:
...
## D-060 H-046 승인(Go) 확정 및 H-047 후속 점검 정책
- Date: 2026-03-10
- Status: Approved (H-046 Close-out / H-047 Scope)
- Decision:
  - H-046 결과를 Main 최종 판단 `Go`로 승인한다.
  - H-046 재집계 결과(`INSUFFICIENT_SAMPLE_RATIO=0.8571`, `SUFFICIENT_DAYS=2`, `executionGapDelta=-23`, `chainShareGapDelta=0.00%p`, 최근 3일 평균 전체 `parseEligibleRunCount=15.3333`)를 기준으로 `resumeDecision=KEEP_FROZEN`을 유지한다.
  - 다음 실행 라운드는 H-047로 고정하고, 최신 시딩 누적/게이트 재집계 + H-036~H-039/H-042/H-043/H-044/H-045/H-046/H-047 readiness 추세 비교로 `RESUME_H024|KEEP_FROZEN` 단일 판정을 재검증한다.
  - H-047에서는 H-046의 배치별 `SEED_TIMESTAMP` 분리 원칙을 유지하되, 신규 증거를 H-046 실행일(`2026-03-09` KST)과 다른 KST 날짜 창에서 확보해 `SUFFICIENT_DAYS`/`INSUFFICIENT_SAMPLE_RATIO` 개선 가능성을 검증한다. 같은 라운드 안에서 다중 날짜 실행이 불가능하면 실제 제약과 추가로 필요한 distinct compliant day 수를 결과 보고에 명시한다.
  - fallback-warning 운영 계약 필드(`signalRecoveryEvidenceLedger[]`, `evidenceAccumulationSummary[]`, `evidenceFreshnessSummary[]`), 임계치/알림 룰 수치(`0.05`, `0.15`, `+0.10p`, `0.10`), `INSUFFICIENT_SAMPLE` 제외 규칙은 변경하지 않는다.
- Rationale: H-046에서 배치별 감사 추적성 공백은 해소됐고 실행량/최근 3일 평균 모수도 일부 개선됐지만, 재개 게이트 4종 중 2종(`INSUFFICIENT_SAMPLE_RATIO`, `SUFFICIENT_DAYS`)은 여전히 미충족이다. 따라서 다음 레버리지는 같은 날 총량 추가보다 일자 분산 실행을 통한 게이트 개선 가능성 검증에 있다.
- Consequence:
  - Main은 다음 실행 라운드를 `coordination/HANDOFFS/H-047-fallback-warning-keep-frozen-resume-readiness-followup-check.md`로 확정한다.
