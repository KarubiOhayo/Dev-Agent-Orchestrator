...
## 현재 우선순위
- [~] H-032 진행중: fallback-warning `KEEP_FROZEN` 상태에서 `LOW_TRAFFIC`/`CHAIN_COVERAGE_GAP` 신호 개선 실증 데이터 확보 계약 정합화(최신 14일/7일 실측 + `signalRecoveryEvidenceLedger[]` + `nextCheckTrigger` 강화)

## Frozen/Backlog
- [ ] H-024 동결: fallback warning 실행량 회복 액션 최소 이행률 하한선/증거 규약 고정
  사유: 트래픽/샘플 미충족(`LOW_TRAFFIC`, `CHAIN_COVERAGE_GAP`) 장기화 + H-031 `KEEP_FROZEN` 유지 판정(`executionGapDelta=+3`, `chainShareGapDelta=0.00%p`, `recoveryActionCompletionRate=0.00`, `blockedActionCount=2`)으로 재개 조건 미충족(H-032 실증 데이터 확보에서 `RESUME_H024` 근거 확보 전까지 동결 유지)
