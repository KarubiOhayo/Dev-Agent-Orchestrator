...
## 현재 우선순위
- [~] H-023 진행중: fallback warning 실행량 회복 액션 이행률 추적/검증 (최근 7일 `executionGap`/`chainShareGap` 축소 추세 검증 + 직접/체인 증량 액션 이행 근거 고정 + 14일 게이트 판정/`READY/HOLD` 입력 강화)
