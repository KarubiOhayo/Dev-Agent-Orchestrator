...
## 현재 우선순위
- [~] H-019 진행중: fallback warning 재보정 착수 가능 시점 재점검 (최신 14일 실측 재집계 + 4개 게이트 충족 여부 재판정 + 임계치 후보 산정 착수 조건 판단)
