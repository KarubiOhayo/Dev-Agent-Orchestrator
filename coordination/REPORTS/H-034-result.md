# H-034 결과 보고서 (fallback-warning `KEEP_FROZEN` 신선 증거 복구 추적 정합화)

## 상태
- 현재 상태: **완료 (운영 문서/야간 템플릿 H-034 계약 동기화 + 테스트 게이트 통과)**
- 실행일(KST): `2026-02-20`
- 점검 구간(KST):
...
## 구현 요약
- `docs/code-agent-api.md`에 H-034 섹션을 추가해 `evidenceFreshnessSummary[]` 필수 필드(`requiredFreshEvidenceCount`, `freshEvidenceCount`, `freshnessRate`, `staleEvidenceCount`, `freshnessStatus`, `refreshAction`, `nextRefreshDueAt`)와 산식/판정 규칙을 고정했습니다.
- 기존 계약(`resumeDecision` 단일값, `signalRecoveryEvidenceLedger[]`, `evidenceAccumulationSummary[]`, 게이트 4개, 임계치/알림 룰 수치, `INSUFFICIENT_SAMPLE` 제외 규칙)은 유지했습니다.
- `coordination/AUTOMATIONS/A-001-nightly-test-report.md`를 H-034 기준으로 동기화하고 `KEEP_FROZEN` 시 `evidenceFreshnessSummary[]` 누락 금지 조건을 추가했습니다.
- `docs/PROJECT_OVERVIEW.md` 완료 항목에 H-034를 반영했습니다.
- 실측값은 `storage/devagent.db`(`runs`, `run_events`)를 KST 기준(`created_at + 9h`)으로 재집계해 사용했습니다.
...
- `latestDecisionReason = KEEP_FROZEN: 게이트 4개 중 2개 미충족 + executionGapDelta=+5 + chainShareGapDelta=0.00%p + evidenceAccumulationSummary(LOW_TRAFFIC=0.50/stale=1, CHAIN_COVERAGE_GAP=0.00) + evidenceFreshnessSummary(LOW_TRAFFIC=0.00, CHAIN_COVERAGE_GAP=0.00) + weeklyComplianceRate=0.00`
...
- 판정 근거:
  ...
  - `evidenceAccumulationSummary`에서 `LOW_TRAFFIC`는 `coverageRate=0.50`이지만 stale 중심(`staleEvidenceCount=1`)이며, `CHAIN_COVERAGE_GAP`은 `coverageRate=0.00`입니다.
  - `evidenceFreshnessSummary`에서 두 신호 모두 `freshnessRate=0.00`, `freshnessStatus=INSUFFICIENT`입니다.
...
- `nextCheckTrigger`:
  ...
  - 우선순위 액션: `LOW_TRAFFIC` fresh runId 증거 확보(우선순위 1), `CHAIN_COVERAGE_GAP` fresh 체인 이벤트 증거 확보(우선순위 2)
...
- `LOW_TRAFFIC`는 누적 증거가 stale 상태(`staleEvidenceCount=1`)라 신선 증거 전환이 지연될 수 있습니다.
- `CHAIN_COVERAGE_GAP`은 누적/신선 증거가 모두 부재(`observedEvidenceCount=0`, `freshEvidenceCount=0`)해 `KEEP_FROZEN` 장기화 위험이 큽니다.
...
## 리뷰 집중 포인트
1. `docs/code-agent-api.md` H-034 섹션이 `evidenceFreshnessSummary[]` 필수 필드/산식(`freshnessRate`) 및 `freshnessStatus` 판정 규칙(`SUFFICIENT/INSUFFICIENT`)을 정확히 반영했는지
2. `coordination/AUTOMATIONS/A-001-nightly-test-report.md`에서 `KEEP_FROZEN` 분기 시 `evidenceFreshnessSummary[]`를 포함한 누락 금지 목록이 반영됐는지
3. H-034 수치/판정(`executionGapDelta=+5`, `chainShareGapDelta=0.00%p`, `LOW_TRAFFIC freshnessRate=0.00(stale=1)`, `CHAIN_COVERAGE_GAP freshnessRate=0.00`, `resumeDecision=KEEP_FROZEN`)이 문서/리포트/릴레이 간 일치하는지
