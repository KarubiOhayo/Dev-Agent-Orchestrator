# H-046 fallback-warning `KEEP_FROZEN` resume readiness next check

Owner: WT-46 (`codex/h046-fallback-warning-keep-frozen-resume-readiness-next-check`)
Priority: High

## 목표
- H-045에서 유지된 `KEEP_FROZEN` 판정을 바탕으로 fail-fast 반복 시딩을 이어가 최신 증거를 추가 누적한다.
- 최신 14일/7일 게이트를 동일 산식으로 재집계하고 `RESUME_H024|KEEP_FROZEN` 단일 판정을 갱신한다.
- H-036~H-039/H-042/H-043/H-044/H-045/H-046 readiness 추세를 단일 표로 비교해 개선/정체/악화 판독 근거를 고정한다.
- 진단/direct/chain 배치별 시딩 산출물을 분리해 감사 추적성과 결과 보고 근거를 강화한다.
...
- 기준 입력:
  ...
  - `coordination/REPORTS/CURRENT_STATUS_2026-03-09.md`
...
- readiness 추세 검증 정책:
  - H-036/H-037/H-038/H-039/H-042/H-043/H-044/H-045/H-046 기준으로 아래 항목의 비교 표를 결과 보고에 포함한다.
...
4. H-036~H-039/H-042/H-043/H-044/H-045/H-046 readiness 추세 비교 표(`INSUFFICIENT_SAMPLE_RATIO`, `SUFFICIENT_DAYS`, `executionGapDelta`, `chainShareGapDelta`, 최근 3일 평균 전체 모수)가 포함된다.
5. fail-fast 실패 원인 분류 표(`rootCause`, `count`, `latestEvidenceRef`, `impact`)가 포함된다.
6. 최신 14일 게이트 4종 PASS/FAIL + 최근 7일/직전 7일 delta(`executionGapDelta`, `chainShareGapDelta`)가 포함된다.
7. `resumeDecision` 단일 판정(`RESUME_H024|KEEP_FROZEN`)과 `unmetReadinessSignals`가 포함된다.
8. fail-fast 실행 중 실패가 발생한 경우, 해당 케이스가 non-zero 종료코드로 처리되었음이 보고된다.
9. 공통 승인 대상 파일(`application.yml`, 공용 모델/계약, 빌드 설정) 변경이 없다.
10. `./gradlew clean test --no-daemon` 통과.
...
## 보고서
- 완료 시 `coordination/REPORTS/H-046-result.md` 생성
- 리뷰 입력 릴레이 `coordination/RELAYS/H-046-executor-to-review.md` 생성
- 필수 포함:
  - 변경 파일 목록
  ...
  - H-036~H-039/H-042/H-043/H-044/H-045/H-046 readiness 추세 비교 표 + 추세 판독
