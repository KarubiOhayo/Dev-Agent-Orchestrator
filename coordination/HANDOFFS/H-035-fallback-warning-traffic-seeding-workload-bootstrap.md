...
- 스크립트 요구사항:
  ...
  - 최소 2개 워크로드를 제공한다.
    ...
    2. Spec -> Code -> Doc/Review 체인 호출 시드
       - 예: `./devagent spec -u "<request>" --chain-to-code true --code-chain-to-doc true --code-chain-to-review true --code-chain-failure-policy PARTIAL_SUCCESS`
...
- 집계/검증:
  ...
  - `run_events`에서 `CHAIN_DOC_*`, `CHAIN_REVIEW_*` 이벤트 발생 여부를 확인한다.
...
2. 결과 보고에 최소 1건 이상의 유효 runId가 포함되고, `CHAIN_DOC_*` 또는 `CHAIN_REVIEW_*` 이벤트 확인 근거가 포함된다.
3. 결과 보고에 최신 14일 게이트 4개 + 최근 7일 대비 변화량(`executionGapDelta`, `chainShareGapDelta`)이 포함된다.
4. `docs/cli-quickstart.md`, `docs/code-agent-api.md`가 H-035 목적(트래픽 생성 우선)과 실행 절차로 동기화된다.
5. fallback-warning 계약 필드 신규 추가 없이 기존 계약을 유지한다.
6. 코드/설정 금지 파일 변경이 없다.
...
- 필수 포함:
  ...
  - `run_events` 근거(`CHAIN_DOC_*`, `CHAIN_REVIEW_*`) 확인 표
