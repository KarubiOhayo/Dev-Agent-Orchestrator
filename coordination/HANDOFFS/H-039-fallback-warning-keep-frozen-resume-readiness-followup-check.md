# H-039 fallback-warning `KEEP_FROZEN` resume readiness follow-up check

Owner: WT-39 (`codex/h039-fallback-warning-keep-frozen-resume-readiness-followup-check`)
Priority: High

## 목표
- H-038에서 이어진 fail-fast 반복 시딩을 유지해 `parseEligibleRunCount`를 추가 누적하고 최신 14일/7일 게이트를 재집계한다.
- 최근 라운드(H-036~H-039) readiness 지표 추세를 동일 산식으로 비교해 `RESUME_H024|KEEP_FROZEN` 단일 판정 근거를 강화한다.
- fail-fast 체인 실패 원인 재발 빈도를 최신 데이터로 갱신하고 완화 가이드 적용 상태를 재확인한다.
...
## 구현 지침
- 기준 입력:
  - `coordination/HANDOFFS/H-038-fallback-warning-keep-frozen-seeding-failure-pattern-followup.md`
  - `coordination/REPORTS/H-038-result.md`
  - `coordination/REPORTS/H-038-review.md`
  - `coordination/RELAYS/H-038-review-to-main.md`
...
- 실행 정책:
  ...
  - chain 배치가 fail-fast로 중단될 경우 동일 파라미터 `SEED_CHAIN_RUNS=1` 재시도로 보강하되, 무한 반복을 피하기 위해 재시도 횟수 상한(권장 8회)을 적용하고 미달 시 사유/증빙/대체 계획을 결과 보고에 남긴다.
- 데이터 검증 정책:
...
- readiness 추세 검증 정책:
  - H-036/H-037/H-038/H-039 4개 라운드 기준으로 아래 항목의 비교 표를 결과 보고에 포함한다.
...
3. H-036~H-039 readiness 추세 비교 표(`INSUFFICIENT_SAMPLE_RATIO`, `SUFFICIENT_DAYS`, `executionGapDelta`, `chainShareGapDelta`, 최근 3일 평균 전체 모수)가 포함된다.
...
## 보고서
- 완료 시 `coordination/REPORTS/H-039-result.md` 생성
- 리뷰 입력 릴레이 `coordination/RELAYS/H-039-executor-to-review.md` 생성
- 필수 포함:
  - 변경 파일 목록
  ...
  - H-036~H-039 readiness 추세 비교 표 + 추세 판독
