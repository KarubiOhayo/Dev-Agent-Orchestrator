# H-049 fallback-warning `KEEP_FROZEN` resume readiness follow-up check

Owner: WT-49 (`codex/h049-fallback-warning-keep-frozen-resume-readiness-followup-check`)
Priority: High

## 목표
- H-048 Executor 결과에서 확인된 개선 신호(`INSUFFICIENT_SAMPLE_RATIO=0.7143`, `SUFFICIENT_DAYS=4`, 최근 3일 평균 전체 `parseEligibleRunCount=30.6667`, `requiredDistinctCompliantDays=3`)를 review gate 통과 후 최신 창에서 재검증한다.
- H-036~H-039/H-042/H-043/H-044/H-045/H-046/H-047/H-048/H-049 readiness 추세를 단일 표로 비교해 개선/정체/악화 판독 근거를 갱신한다.
- H-048 최신 증거일(`2026-03-11` KST)과 다른 KST 날짜 창의 신규 시딩 증거를 확보해 `SUFFICIENT_DAYS`, `INSUFFICIENT_SAMPLE_RATIO`, `requiredDistinctCompliantDays` 추가 개선 가능성을 검증한다.
...
- 착수 게이트:
  ...
  - Main이 최신 상태 문서에서 H-048 보류 해제를 확인했는지 검토한다.
- 기준 입력:
  - `coordination/REPORTS/H-048-result.md`
  - `coordination/REPORTS/H-048-review.md`
  - `coordination/RELAYS/H-048-review-to-main.md`
  - `coordination/REPORTS/CURRENT_STATUS_2026-03-17.md` 또는 후속 최신 상태 보고
...
- 실행 정책:
  - 신규 시딩 증거는 H-048 최신 증거일(`2026-03-11` KST)과 다른 KST 날짜 창에서 최소 1회 이상 확보하는 것을 우선한다.
  ...
  - 같은 라운드 안에서 다중 KST 날짜 실행이 현실적으로 불가능하면, 타임스탬프를 임의로 조작하지 말고 실제 제약과 추가로 필요한 최소 distinct compliant day 수를 결과 보고에 명시한다.
- 데이터 검증 정책:
...
- readiness 추세 검증 정책:
  - H-036/H-037/H-038/H-039/H-042/H-043/H-044/H-045/H-046/H-047/H-048/H-049 기준으로 아래 항목의 비교 표를 결과 보고에 포함한다.
...
## 수용 기준
1. 착수 전 H-048 review gate 완료 여부가 결과 보고에 명시된다.
2. 신규 시딩을 수행했다면 direct/chain 실행 결과가 결과 보고에 포함되고, 최소 1건 이상의 유효 chain 증거(`CHAIN_DOC_DONE` 또는 `CHAIN_REVIEW_DONE`)가 `codeRunId` 기준으로 제시된다.
3. 기본 목표(총 9회) 달성 여부가 명시되고, 미달 시 사유/증빙/대체 계획이 함께 보고된다.
4. 진단/direct/chain 각 배치의 `SEED_TIMESTAMP`(또는 동등한 고유 suffix)가 분리돼 배치별 `summary.json`, `before/after`, `records.jsonl`, `log`를 독립 검증할 수 있다.
5. 최근 14일 일자별 `actualTotalRuns`, `actualChainRuns`, `parseEligibleRunCount`, `dailyCompliance`, `sufficientSample` 표가 포함된다.
6. `INSUFFICIENT_SAMPLE_RATIO <= 0.50`, `SUFFICIENT_DAYS >= 7` 도달을 위해 추가로 필요한 최소 distinct compliant day 수가 결과 보고에 포함된다.
7. H-036~H-039/H-042/H-043/H-044/H-045/H-046/H-047/H-048/H-049 readiness 추세 비교 표(`INSUFFICIENT_SAMPLE_RATIO`, `SUFFICIENT_DAYS`, `executionGapDelta`, `chainShareGapDelta`, 최근 3일 평균 전체 모수)가 포함된다.
8. fail-fast 실패 원인 분류 표(`rootCause`, `count`, `latestEvidenceRef`, `impact`)가 포함된다.
9. 최신 14일 게이트 4종 PASS/FAIL + 최근 7일/직전 7일 delta(`executionGapDelta`, `chainShareGapDelta`)가 포함된다.
10. `resumeDecision` 단일 판정(`RESUME_H024|KEEP_FROZEN`)과 `unmetReadinessSignals`가 포함된다.
11. fail-fast 실행 중 실패가 발생한 경우, 해당 케이스가 non-zero 종료코드로 처리되었음이 보고된다.
12. 공통 승인 대상 파일(`application.yml`, 공용 모델/계약, 빌드 설정) 변경이 없다.
13. `./gradlew clean test --no-daemon` 통과.
...
## 보고서
- 완료 시 `coordination/REPORTS/H-049-result.md` 생성
- 리뷰 입력 릴레이 `coordination/RELAYS/H-049-executor-to-review.md` 생성
- 필수 포함:
  - 변경 파일 목록
  - H-048 review gate 충족 여부 요약
  ...
  - H-036~H-039/H-042/H-043/H-044/H-045/H-046/H-047/H-048/H-049 readiness 추세 비교 표 + 추세 판독
