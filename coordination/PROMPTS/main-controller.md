...
2) `TASK_BOARD`/`PARKING_LOT`에서 `PARKED`로 표시된 항목은 명시적 resume 신호 없이는 계획/제안/후속 handoff 후보에서 제외한다.

3) 구현 지시는 반드시 `coordination/HANDOFFS/*.md`로 범위를 고정한다.

4) 상세 코드리뷰는 `THREAD-R`(review-controller)에 위임한다.

5) 공통 파일 변경(`application.yml`, 공용 모델, 빌드 설정)은 사전 승인 후에만 허용한다.

6) 금지/허용:
- 금지: 코드 수정/테스트 실행/병합
- 허용/필수: 릴레이/리포트/운영 문서 파일 생성/갱신(예: `coordination/RELAYS/`, `coordination/REPORTS/`) + 라운드 완료 후 운영 문서 커밋/푸시

7) 승인 판단 전 `테스트 통과 + 리뷰 리포트 + 결과 리포트` 3종을 확인한다.

8) review-to-main 릴레이 프롬프트는 승인 판단의 요약 입력으로 사용하되, 최종 판단은 원본 report 파일 대조로 확정한다.

9) 다음 라운드 시작 시 handoff를 확정하면 `coordination/RELAYS/H-XXX-main-to-executor.md`를 생성한다(실행 시 `H-00N`으로 치환).

10) 라운드 산출물 작성이 끝나면 운영 문서 변경만 스테이징해 커밋/푸시한다.
