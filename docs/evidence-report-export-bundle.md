...
빠른 발송 판단만 필요하면 [`docs/proof-package-delivery-checklist.md`](./proof-package-delivery-checklist.md)를 먼저 보고, 이 문서는 `README -> case study -> walkthrough -> evidence bundle` starter set의 네 번째 문서이자 상세 bundle mapping / read-next reference로 사용한다.
...
|---|---|---|---|
| starter set | [`README.md`](../README.md), [`docs/portfolio-case-study.md`](./portfolio-case-study.md), [`docs/demo-showcase-walkthrough.md`](./demo-showcase-walkthrough.md), [`docs/evidence-report-export-bundle.md`](./evidence-report-export-bundle.md) | [`docs/proof-package-delivery-checklist.md`](./proof-package-delivery-checklist.md) 기준으로 첫 follow-up 메일/메시지에서 기본으로 보낸다 | 저장소가 무엇이고, 왜 orchestration layer 관점에서 의미가 있으며, 어떤 순서로 읽어야 하는지 |
...
| starter set | Detailed mapping / read-next reference | [이 문서](./evidence-report-export-bundle.md) | canonical starter set 순서를 뒤집지 않고, 네 번째 문서에서 add-on 분기와 proof mapping을 이어 설명한다 | External default |
...
실제 repo에 export 폴더를 새로 만들 필요는 없다. 아래는 [`docs/proof-package-delivery-checklist.md`](./proof-package-delivery-checklist.md) 기준으로 외부 전달용 묶음을 준비할 때의 권장 순서다.
...
2. 세션 직후에는 [`docs/proof-package-delivery-checklist.md`](./proof-package-delivery-checklist.md)로 순서를 확인한 뒤 starter set만 먼저 보낸다. 실제 첨부 순서는 `README -> case study -> walkthrough -> evidence bundle`로 유지하고, 이 문서는 네 번째 문서에서 add-on 분기와 proof mapping을 이어 설명하는 역할로만 둔다.
...
- [`README.md`](../README.md), [`docs/portfolio-case-study.md`](./portfolio-case-study.md), [`docs/demo-showcase-walkthrough.md`](./demo-showcase-walkthrough.md), [`docs/evidence-report-export-bundle.md`](./evidence-report-export-bundle.md) 중 하나라도 핵심 클레임이나 starter set 순서가 바뀌면 이 문서의 mapping table과 starter set 설명을 같은 라운드에서 함께 갱신한다.
...
- [`docs/proof-package-delivery-checklist.md`](./proof-package-delivery-checklist.md)와 이 문서가 같은 checklist authority / add-on 역할 분리를 유지하는지 같이 본다.
