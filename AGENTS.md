...
## 1) 3-스레드 역할 정의
- Main-Control: 읽기 전용. 라운드 계획, 승인/보류 판단, handoff/문서 동기화 담당.
- Review-Control: 읽기 전용. 결과물 리뷰 전담(P1 > P2 > P3, 파일/라인 근거 필수).
- Executor: 구현 전용. handoff 범위 내 수정/테스트/결과 보고 담당.
...
- Main/Review는 구현 실행 스레드가 아니므로 코드 수정/커밋/구현 실행을 하지 않는다.
